context("CALC$utility")

test_that("CALC$utility$compare_matrix Null for identical matricies", {
  # m1 = matrix(data = c(1:9), nrow = 3, ncol = 3)
  # m2 = matrix(data = c(1:9), nrow = 3, ncol = 3)
  
  m1 = tibble(x=c(1, 2, 3), y=c(4,5,6))
  m2 = tibble(x=c(1, 2, 3), y=c(4,5,6))
  
  # expect_that( CALC$utility$compare_matrix(m1, m2), expect_null())
  expect_null(CALC$utility$compare_matrix(m1, m2))
})


test_that("CALC$utility$compare_matrix returns List of differences for non identical matricies", {
  # m1 = matrix(data = c(1:9), nrow = 3, ncol = 3)
  # m2 = matrix(data = c(1:9), nrow = 3, ncol = 3)
  
  m1 = tibble(x=c(1, 2, 3), y=c(4,5,6))
  m2 = tibble(x=c(2, 2, 3), y=c(4,5,6))
  
  # expect_that( CALC$utility$compare_matrix(m1, m2), expect_null())
  expect_equal(CALC$utility$compare_matrix(m1, m2), list(list(r=1, c=1, v1=1, v2=2)) ) 
})
